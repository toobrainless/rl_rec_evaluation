...
def preprocessing(
    ...
    if users_sample is not None:
        ...
        sampled_users = np.random.choice(
            data["user_id"].unique(), size=users_sample, replace=False
        )
    ...
    if core:
        ...
        step = 1
        while (
            data["user_id"].value_counts().min() < min_len
        ...
        data = filter_iteractions(data, 30)
        data = filter_items(data, 10)
        if drop_repeats:
            data = drop_repeated_items(data)
        data = drop_short_sequences(data, min_len)
        data = filter_items(data, item_min_count)
        data = drop_short_sequences(data, 2)
...
def rename(data, user_id="user_id", item_id="item_id", timestamp="timestamp"):
    ...
    columns_rename = {user_id: "user_id", item_id: "item_id", timestamp: "timestamp"}
...
def filter_iteractions(data, min_duration, duration="play_duration"):
