...
class Evaluator:
    ...
    def downvote_seen_items(self, scores: np.ndarray, history: pd.DataFrame) -> None:
        ...
        seen_idx_flat = np.ravel_multi_index(
            (user_idx, item_idx), scores_downvoted.shape
        )
        ...
        for k in self.top_k:
            result.update(
                self._compute_accuracy_metrics(
                    holdout, recs, recs_array, k, n_test_users
                )
            )
            if train is not None:
                result.update(
                    self._compute_beyond_accuracy_metrics(recs_array, train, k)
                )
        ...
        for metric in self.metrics:
            ...
            if metric in ["hr_k", "mrr_k"]:
                result[f"{metric_name}@{k}"] = np.round(
                    self.get_custom_hr_mrr(
                        metric, holdout, recs_array, n_test_users, k
                    ),
                    6,
                )
            else:
    ...
    def _compute_beyond_accuracy_metrics(
        self, recs_array: np.ndarray, train: pd.DataFrame, k: int
    ) -> dict:
        result = {}
        for metric in self.metrics_beyond_accuracy:
            ...
            if metric == "catalog_coverage":
                result[f"{metric_name}@{k}"] = np.round(
                    np.unique(recs_array[:, :k]).size / train[self.col_item].nunique(),
                    6,
                )
            else:
                metric_func = getattr(python_evaluation, metric)
    ...
    def get_custom_hr_mrr(
        ...
        hits_mask = np.array(
            [
                np.isin(recs_array[i, :k], holdout_array[i])
                for i in range(recs_array.shape[0])
            ]
        )
    ...
    def _datarecs2array(self, recs: pd.DataFrame) -> np.ndarray:
        ...
        return np.vstack(
            recs.sort_values(
                by=[self.col_user, self.col_score], ascending=[True, False]
            )
